package com.p6spy.engine.oracle.lob;
 ...
 * @see oracle.sql.CLOB
 */
public class OracleLobForP6spyHandler extends AbstractLobHandler {
	...
	protected class OracleLobCreator implements LobCreator {
		...
		protected Connection getOracleConnection(PreparedStatement ps)
			...
			Connection con =  (nativeJdbcExtractor != null) ?
					nativeJdbcExtractor.getNativeConnectionFromStatement(ps) : ps.getConnection();
			//Add by linzhw,2010-8-13, add checking the Connect instance to do the different processes.
			if(con instanceof P6Connection) {
				return ((P6DatabaseMetaData)((P6Connection)con).getMetaData()).getJDBC().getConnection();
			}else {
				return con;
			}
		}
